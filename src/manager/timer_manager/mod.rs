...
use std::{sync::RwLock, time::Duration, vec::Vec,boxed::Box};

lazy_static! {
    static ref TIMER_TASK: RwLock<TimerManager> = RwLock::new(TimerManager {
...
impl TimerManager {
    ...
    {
        TIMER_TASK.write().unwrap().inner_timers.push(Timer::new(
            duration,
            Box::new(callback),
            parent,
        ));
    ...
    // 检测定时器是否应该触发
    pub fn check_timer() {
        let mut writer = TIMER_TASK.write().unwrap();
        //此处触发定时器，若定时器被触发，则移除
        writer.inner_timers.retain_mut(|x| !x.check());
    }

    // 取消掉一个unit的所有定时任务，一般在unit启动失败或者退出unit时进行的操作
    pub fn cancel_timer(unit_id: usize) {
        TIMER_TASK.write().unwrap().inner_timers.retain(|x| x.parent() == unit_id)
    }
}
